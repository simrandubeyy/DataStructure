...
int main()
{
	...
	{	
		...
		else
		{
			...
			while(hash[permant_modulo]>0)
			{
			    int square=count*count;
				permant_modulo=modulo+square;
				count++;
				while(permant_modulo>9)
				{
					permant_modulo=permant_modulo%10;
				}
			}
			
			if(hash[permant_modulo]==0)
			{
				hash[permant_modulo]=arr[i];
			}
		}		    
	}
